...
API_URL = "https://app.omie.com.br/api/v1/financas/contareceber/"
CLIENTES_API_URL = "https://app.omie.com.br/api/v1/geral/clientes/"
REGISTROS_POR_PAGINA = 20
MAX_TENTATIVAS = 5
INTERVALO_ENTRE_PAGINAS = 0.3
LOG_DIR = BASE_DIR / "logs"
LOG_FILE = LOG_DIR / "listar_contas_receber_omie.log"
CHECKPOINT_FILE = LOG_DIR / "listar_contas_receber_omie.checkpoint.json"
...
def buscar_clientes_ausentes(contas: list[Any]) -> list[int]:
    codigos = sorted(
        {
            codigo
            for conta in contas
...
def garantir_clientes_cadastrados(contas: list[Any]) -> int:
    codigos_ausentes = buscar_clientes_ausentes(contas)
    if not codigos_ausentes:
        return 0

    logging.warning(
        "Encontrados %s clientes ausentes em raw_omie_clientes: %s. "
        "Consultando cadastros antes de gravar contas a receber.",
...
def salvar_contas_receber_no_banco(dados: dict[str, Any]) -> int:
    contas = dados.get("conta_receber_cadastro", [])
    if not isinstance(contas, list):
        raise RuntimeError("Resposta inesperada: campo 'conta_receber_cadastro' nao e uma lista.")

    clientes_inseridos = garantir_clientes_cadastrados(contas)
    if clientes_inseridos:
        logging.info(
            "%s clientes ausentes foram gravados em raw_omie_clientes.",
            clientes_inseridos,
        )
    ...
    for conta in contas:
        ...
        boleto = objeto(conta.get("boleto"))
        ...
        registros.append(
            {
                ...
                "n_cod_os": inteiro(conta.get("nCodOS")),
                ...
                "numero_pedido": texto(conta.get("numero_pedido")),
                "operacao": texto(conta.get("operacao")),
                "tipo_agrupamento": texto(conta.get("tipo_agrupamento")),
                "data_emissao": texto(conta.get("data_emissao")),
                "data_previsao": texto(conta.get("data_previsao")),
                "data_registro": texto(conta.get("data_registro")),
                ...
                "bloquear_baixa": texto(conta.get("bloquear_baixa")),
                "bloquear_exclusao": texto(conta.get("bloquear_exclusao")),
                "boleto_gerado": texto(boleto.get("cGerado")),
                "boleto_numero_bancario": texto(boleto.get("cNumBancario")),
                "boleto_numero": texto(boleto.get("cNumBoleto")),
                "boleto_data_emissao": texto(boleto.get("dDtEmBol")),
                "boleto_percentual_juros": decimal(boleto.get("nPerJuros")),
                "boleto_percentual_multa": decimal(boleto.get("nPerMulta")),
                ...
                "boleto_json": json_objeto(conta.get("boleto")),
    ...
    sql = f"""
        INSERT INTO raw_omie_contas_receber ({lista_colunas})
        VALUES ({placeholders})
...
def extrair_e_salvar(
    ...
    while True:
        ...
        try:
            ...
            logging.info("Pagina %s sem contas a receber. Fim da consulta.", pagina)
            break
        ...
        salvos = salvar_contas_receber_no_banco(dados)
        total += salvos
        if pagina_final is None:
            salvar_checkpoint(pagina, total)
        logging.info(
            "Pagina %s/%s processada: %s contas a receber gravadas no banco.",
            pagina,
            total_paginas or "?",
            salvos,
        )
...
def argumentos() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Extrai contas a receber da API Omie para o MySQL.")
    parser.add_argument(
        "--pagina-inicial",
        type=int,
...
def main() -> int:
    ...
    logging.info(
        "Extracao finalizada: %s contas a receber gravadas no banco %s.",
        total,
        MYSQL_DATABASE,
    )
    return 0
