...
API_URL = "https://app.omie.com.br/api/v1/geral/cidades/"
REGISTROS_POR_PAGINA = 50
MAX_TENTATIVAS = 5
INTERVALO_ENTRE_PAGINAS = 0.3
LOG_DIR = BASE_DIR / "logs"
LOG_FILE = LOG_DIR / "listar_cidades_omie.log"
CHECKPOINT_FILE = LOG_DIR / "listar_cidades_omie.checkpoint.json"
...
def salvar_cidades_no_banco(dados: dict[str, Any]) -> int:
    cidades = dados.get("lista_cidades", [])
    if not isinstance(cidades, list):
        raise RuntimeError("Resposta inesperada: campo 'lista_cidades' nao e uma lista.")
    ...
    for cidade in cidades:
        ...
        registros.append(
            {
                "codigo": codigo,
                "nome": texto(cidade.get("cNome")),
                "uf": texto(cidade.get("cUF")),
                "codigo_ibge": texto(cidade.get("nCodIBGE")),
                "codigo_siafi": inteiro(cidade.get("nCodSIAFI")),
                ...
                "dados_json": json.dumps(cidade, ensure_ascii=False),
                "dados_flat_json": json.dumps(achatar_json(cidade), ensure_ascii=False),
            }
        )
    ...
    sql = f"""
        INSERT INTO raw_omie_cidades ({lista_colunas})
        VALUES ({placeholders})
...
def extrair_e_salvar(pagina_inicial: int | None = None) -> int:
    ...
    while True:
        try:
            ...
            logging.info("Pagina %s sem cidades. Fim da consulta.", pagina)
            break
        ...
        salvos = salvar_cidades_no_banco(dados)
        total += salvos
        salvar_checkpoint(pagina, total)
        logging.info(
            "Pagina %s/%s processada: %s cidades gravadas no banco.",
            pagina,
            total_paginas or "?",
            salvos,
        )
...
def argumentos() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Extrai cidades da API Omie para o MySQL.")
    parser.add_argument(
        "--pagina-inicial",
        type=int,
...
def main() -> int:
    ...
    logging.info(
        "Extracao finalizada: %s cidades gravadas no banco %s.",
        total,
        MYSQL_DATABASE,
    )
    return 0
